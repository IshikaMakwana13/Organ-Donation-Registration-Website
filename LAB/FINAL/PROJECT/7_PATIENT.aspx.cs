...
public partial class _4_PATIENT : System.Web.UI.Page
{
    SqlCommand cmd;
    string q;
    SqlConnection con;
    ...
    // FIX #10: Parameterized query â€” was vulnerable to SQL injection
    protected void search_btn_Click(object sender, EventArgs e)
    {
        try
        {
            SqlCommand searchCmd = new SqlCommand("SELECT * FROM organpatient WHERE p_city LIKE @city", con);
    ...
    protected void search_btn_Click_state(object sender, EventArgs e)
    {
        try
        {
            SqlCommand searchCmd = new SqlCommand("SELECT * FROM organpatient WHERE p_state LIKE @state", con);
    ...
    protected void search_btn_Click_organ(object sender, EventArgs e)
    {
        try
        {
            SqlCommand searchCmd = new SqlCommand("SELECT * FROM organpatient WHERE p_organ LIKE @organ", con);
    ...
    protected void search_btn_Click_gender(object sender, EventArgs e)
    {
        try
        {
            SqlCommand searchCmd = new SqlCommand("SELECT * FROM organpatient WHERE p_gender LIKE @gender", con);
